...
export class PivotComponent implements OnInit {
  ...
  // Maintain original order for available fields
  readonly fieldOrder = ['Region', 'Branch', 'Year', 'Month', 'Category', 'Product', 'Sales', 'Profit', 'Margin', 'Qty'];
  ...
  removeGroup(field: string, target: 'row' | 'pivot' | 'value') {
    ...
    if (!this.availableGroupingFields.includes(field)) {
      ...
      // Re-sort available fields to maintain the original column order
      this.availableGroupingFields.sort((a, b) => this.fieldOrder.indexOf(a) - this.fieldOrder.indexOf(b));
    }
